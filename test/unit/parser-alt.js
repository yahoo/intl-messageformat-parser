...
var parser = require('../../src/parser-alt');
...
describe('parse()', function () {
    ...
    describe('parse("{numPhotos, plural, =0{no photos} =1{one photo} other{# photos}}")', function () {
        ...
        it('should contain nested `messageFormatPattern` values for each option', function () {
            ...
            expect(options[2].value.elements[0].type).to.equal('pluralNumberElement');
            expect(options[2].value.elements[1].value).to.equal(' photos');
        });
    });
    ...
    describe('escaping', function () {
        it('should allow escaping of syntax chars via `\'`', function () {
            ...
            expect(ast.elements[0].format.options[0].value.elements[0].type).to.equal('pluralNumberElement');
            expect(ast.elements[0].format.options[0].value.elements[1].value).to.equal('#');
        });
    });
});
